...
public class Analysis {
	...
	public String run(PlatformInfo p) {
		...
		case 1004:	//DA test for any scheduling (applicable for both Uni- and Multiprocessor)
			schedulable=DATest_Multi_V2();
			break;
			
		case 1005:	//RTA test for RM
	...
	public int DATest_Multi_V1()
	{
		...
		{		
			...
			if(Math.floor(sum/pInfo.numProcessor)+task_k.execTime>task_k.Deadline) return 0;
		}	
		return 1;
	}
	
	public int DATest_Multi_V2()
	{
	...
	{						
		...
		{
			Task task_k = pInfo.tasks.get(k);
			int rk= (int)task_k.execTime;
			boolean pass=false;
			
			while (rk<= (int)task_k.Deadline && pass==false) 
			{
				int sum=0;
				for (int i=0; i<pInfo.numTask; i++) 
				{
					Task task_i = pInfo.tasks.get(i);
					if (k>i) 
						sum+=Math.min(workloadWC(task_i.Period, task_i.execTime, task_i.Deadline, rk), rk-(int)task_k.execTime+1);
				}	
				int new_rk = (int)Math.floor(sum/pInfo.numProcessor)+(int)task_k.execTime;
				if (rk==new_rk) pass=true;
				else rk = new_rk;		
			}			
			if (pass==false) 
				return 0;
	...
	public int workloadWC(double T, double C, double D, double L) {
		...
		return (int)Math.floor(N)*(int)C+Math.min((int)C, (int)L+(int)D-(int)C-N*(int)T);
	}	
}